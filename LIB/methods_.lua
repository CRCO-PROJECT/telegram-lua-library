...
check_group = function(chat_id)
    assert(chat_id, printRed"Missing required variable chat_id")

   if  getChat(chat_id).result then
    return true
   else
 return false
end
...
ResponseParameters = function(chat_id,time)
    url = bot_url..MAIN.token..'/ResponseParameters?migrate_to_chat_id='..chat_id..'&retry_after='..time
   return REQ(url)
end 
...
prs,
    ...
    url = bot_url..MAIN.token..'/restrictChatMember?chat_id='..chat_id..'&user_id='..user_id..'&permissions='..prs..'&until_date='..untildate                                         
        return REQ(url)
end
...
setChatPermissions = function(chat_id
...
assert(chat_id, printRed"Missing required variable chat_id")
url = bot_url..MAIN.token..'/setChatPermissions?chat_id='..chat_id..'&permissions='..chatprs
  print(url)
return REQ(url)
end
...
UTI.conv_splitnumber = conv_splitnumber
          ...
          MAIN.ResponseParameters = ResponseParameters
...
MAIN.editMessageText = editMessageText 
    MAIN.editInlineQuery = editInlineQuery
            MAIN.sendMedia = sendMedia
                    ...
                    MAIN.setChatPermissions = setChatPermissions
